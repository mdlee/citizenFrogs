...
% MCMC properties
engine = 'jags';
params = {'alpha', 'beta', 'tau', 'stepArrive', 'stepLeave', 'covSocial'};
...
% nBurnin    = 1;   % number of discarded burn-in samples
% nSamples   = 1e1;   % number of collected samples
...
for dataIdx = 1:numel(dataList)
   ...
   switch dataName
      ...
      case 'citizenFrogsSecondDataSet'
         stimuliKeep = 481; % 481; % 1260 is full for data set 1, 481 for dataset 2
         srtFrg = 1:9;
         spLoc = 1:9;
         legLoc = 9;
   ...
   modelName = 'oneHighThresholdNoDifferencesAsymmetricDynamicsCovariance_n';
   data = struct(...
      'nStimuli'    , d.nStimuli       , ...
      'nPeople'     , d.nPeople        , ...
      'nTrials'     , d.nTrials   , ...
   ...
   for figureIdx = 1:numel(figureList)
      ...
      switch figureList{figureIdx}
         case 'ROCs'
            ...
            tauEnvironment = get_matrix_from_coda(chains, 'tau');
            ...
            drawFourROCs(d, tauCovariance, tauEnvironment, tauCognitive, vote, spLoc, legLoc, figSize, slideLeft, pantone);
