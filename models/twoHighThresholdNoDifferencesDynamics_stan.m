%% Two high threshold model for citizen frog aggregation
...
% MCMC properties
engine = 'stan';
params = {'alpha', 'beta', 'tau_prob_state_1', 'gamma'};
...
for dataIdx = 1:numel(dataList)
  ...
  modelName = 'twoHighThresholdNoDifferencesDynamics_n';
  data = struct(...
    'nStimuli'    , d.nStimuli       , ...
    'nPeople'     , d.nPeople        , ...
    'nTrials'     , d.nTrials   , ...
  ...
  tau = reshape(codatable(chains, 'tau', @mean), [], d.nFrogs);
