function  drawROCs_2(d, chains, tau, vote, spLoc, legLoc, figSize, slideLeft, pantone)
...
for frogIdx = 1:d.nFrogs
 ...
 if ~isempty(tau) & spLoc(frogIdx)==legLoc
    L = legend(H, labs, ...
      'box', 'off', ...
      'fontsize', fontSize, ...
      'location', 'northeast');
   set(L, 'pos', get(L, 'pos') + [0.2 0 0 0]);
 end

end

if ~isempty(tau)  & ~ismember(legLoc, spLoc)
